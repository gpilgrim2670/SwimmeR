...
## Why Two Functions?
It would be possible to combine `Read_Results` and `Swim_Parse` together into one function.  Doing so would make troubleshooting typos in the original source file more difficult, and typos can be a major issue when reading outside sources.  Having two functions means that if the output of `Swim_Parse` are not as expected the output of `Read_Results` can be inspected for issues.  Additionally `Read_Results` and `Swim_Parse` work well with `map` functions from `purrr`, so they can be used to read in data from a list of sources.  Separating the web or document scraping aspect from data cleaning is wise, both in terms of managing compuational resources and from a data science perspective.  One can certainly use `Swim_Parse(Read_Results(file))` and effectively combine the two functions.

# Formatting Swimming Times

Once results are captured in R as tidy dataframes the real fun can begin - but there's another problem.  Times in swimming are recorded as minutes:seconds.hundreths.  This is fine when a time is less than a minute, because `59.99` can be of class `numeric` in R, but times greater than or equal to a minute `1:00.00` are stuck as class `character`.  `SwimmeR` provides two functions, `sec_format` and `mmss_format` to convert between times as seconds (class `numerci`, for doing math), and times as minutes:seconds.hundreths (class `character`), for swimming-specific display.
...
Included in `SwimmeR` is `King200Breast`, containing all Lilly King's 200 Breaststroke times for her NCAA career.  Times are recorded in standard minutes:seconds.hundreths format (class `character`) .  We can use `sec_format` to format them as seconds (class `numeric`), and `mmss_format` to go back to minutes:seconds.hundreths.  Both functions work well with the `tidyverse` packages.
