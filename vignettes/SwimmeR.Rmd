...
Lilly has 4 different teams, but all of them are actually the same.  Similarly Sully has two teams, but actually only one.  Using `get_mode` to return the most frequently occurring team for each swimmer is easier than manually specifying every swimmer's team.
...
# Drawing brackets
To aid in making single elimination brackets for tournaments and shoot-outs `SwimmeR` has `draw_bracket`. Any number of teams between 5 and 64 can be used, with byes automatically asigned to higher seeds.
```{r brackets 1}
