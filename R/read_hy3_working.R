...
# df <- read_delim("inst/extdata/Meet Results-2020 NI HAST IMX Madness-09Mar2020-001.hy3", delim = "\\s\\2", col_names = FALSE)
# df <- read_delim("inst/extdata/Meet Results-2020 NI Champs Qualifier UNAC-28Feb2020-001.hy3", delim = "\\s\\2", col_names = FALSE)
#
...
#
# entry_3 <- data.frame(entry_2, stringsAsFactors = FALSE) %>%
#   t()
#
# rownames(entry_3) <- NULL
#
# entry_4 <- data.frame(entry_3, stringsAsFactors = FALSE)
# entry_4 <- entry_4[c("X2","X3","X9")]
# colnames(entry_4) <- c("ID", "Event", "Seed_Time")
# entry_4$Row_Numb <- as.numeric(rows)
#
#
# entry_5 <- entry_4 %>%
#   group_by(ID, Event, Seed_Time) %>%
#   summarise(Row_Numb = min(as.numeric(Row_Numb))) %>%
#   arrange(Row_Numb) %>%
#   ungroup()
#
# entry <- entry_5 %>%
...
#   mutate(Gender = case_when(str_detect(ID, "MB$|MM$") ~ "M",
#                             str_detect(ID, "FG$|FW$") ~ "F")) %>%
...
#   map(head, 1)
#
# if(length(prelims_3) == 0){
#   prelims_3 <- rep(NA, length(rows))
# }
#
...
#
#
# # Adding in finals times
# i <- cut(finals$Row_Numb, t(entry[,6:7]))
# levels(i)[c(FALSE,TRUE)] = NA
# entry$Finals_Time[i] <- finals$Finals_Time
#
# # Adding in prelims times
# i <- cut(prelims$Row_Numb, t(entry[,6:7]))
# levels(i)[c(FALSE,TRUE)] = NA
# entry$Prelims_Time[i] <- prelims$Prelims_Time
#
#
...
#   map(head, 7)
#
# swimmer_3 <- data.frame(swimmer_3, stringsAsFactors = FALSE) %>%
#   t()
#
# rownames(swimmer_3) <- NULL
#
# swimmer_4 <- data.frame(swimmer_3, stringsAsFactors = FALSE)
#
# swimmer_5 <- swimmer_4 %>%
#   mutate(ID = X2,
#          First = X3,
#          USA_ID = case_when(str_detect(X4, "[A-Z]{3,}") ~ X4,
#                             str_detect(X5, "[A-Z]{3,}") ~ X5),
#          Birthdate = case_when(str_detect(X5, "[A-Z]{3,}") == FALSE & str_length(X5) >= 6 ~ X5,
#                                str_detect(X6, "[A-Z]{3,}") == FALSE & str_length(X6) >= 6 ~ X6),
#          Grade = case_when(str_length(X5) < 6 & str_length(X5) >= 1 & X5 != "0" & str_detect(X4, "[A-Z]{3,}") == FALSE ~ X5,
#                            str_length(X6) < 6 & str_length(X6) >= 1 & X6 != "0" ~ X6,
#                            str_length(X6) >= 6 & str_length(X7) < 6 & X7 != "0" ~ X7)) %>%
#   mutate(Row_Numb = as.numeric(rows)) %>%
#   select(ID, First, USA_ID, Birthdate, Grade, Row_Numb)
#
#
#
...
#
# team_2 <-
#   unlist(purrr::map(team_1, stringr::str_split, "\\s{2,}"), recursive = FALSE) %>%
#   map(unique)
#
# rows <- team_2 %>%
#   map(tail, 1) %>%
...
#   mutate(School = str_remove(School, "^C1[A-Z]{1,} ")) %>%
#   mutate(Row_Min = as.numeric(Row_Numb),
#          Row_Max = dplyr::lead(Row_Min, 1L, default = length(as_lines_list_2)) - 1,)
#
#
...
#   map(head, 9)
#
# relay_3 <- data.frame(relay_3, stringsAsFactors = FALSE) %>%
#   t()
#
# rownames(relay_3) <- NULL
#
# relay_4 <- data.frame(relay_3, stringsAsFactors = FALSE)
#
# # relay_length_14 <- relay_2[purrr::map(relay_2, length) == 14]
# # relay_length_15 <- relay_2[purrr::map(relay_2, length) == 15]
# #
# # relay_14 <- data.frame(relay_length_14, stringsAsFactors = FALSE) %>%
# #   t()
# # relay_15 <- data.frame(relay_length_15, stringsAsFactors = FALSE) %>%
# #   t()
# #
# # rownames(relay_14) <- NULL
# # rownames(relay_15) <- NULL
#
# relay_5 <- relay_4[c("X1", "X2", "X3", "X4","X8", "X9")]
#
# colnames(relay_5) <- c("Team", "Relay_Rank", "ID", "Event", "X8", "X9")
# relay_5$Row_Numb <- as.numeric(rows)
#
# relay_6 <- relay_5 %>%
#   mutate(Seed_Time = case_when(str_detect(X8, "\\d{2,}\\.\\d{2,}") ~ X8,
#                                str_detect(X9, "\\d{2,}\\.\\d{2,}") ~ X9,
#                                TRUE ~ "NA")) %>%
#   na_if("^NA$") %>%
#   select(-X8, -X9)
#
# # relay_14 <- data.frame(relay_14, stringsAsFactors = FALSE)[c("X1", "X2", "X4","X9","X14")]
# # colnames(relay_14) <- c("Team", "Relay_Rank", "Event", "Seed_Time", "Row_Numb")
# #
# # relay_15 <- data.frame(relay_15, stringsAsFactors = FALSE)[c("X1", "X2", "X4","X9","X15")]
# # colnames(relay_15) <- c("Team", "Relay_Rank", "Event", "Seed_Time", "Row_Numb")
#
# # relay <- bind_rows(relay_14, relay_15) %>%
# relay <- relay_6 %>%
#   group_by(Team, Relay_Rank, Event, Seed_Time, ID) %>%
#   summarise(Row_Numb = min(as.numeric(Row_Numb), na.rm = TRUE)) %>%
#   arrange(Row_Numb) %>%
#   ungroup()
#
# relay <- relay %>%
#   mutate(Gender = case_when(str_detect(ID, "MB$|MM$|MXX$") ~ "M",
#                             str_detect(ID, "FG$|FW$|FXX$") ~ "F")) %>%
#   mutate(Course = str_extract(Seed_Time, "[:alpha:]$"),
#          Course = case_when(Course == "Y" ~ "Yard",
#                             Course == "M" ~ "Meter"),
...
#   mutate(Finals_Time = NA,
#          Prelims_Time = NA) %>%
#   select(-Course, -Team, -Relay_Rank, -ID)
#
#
...
#   map(head, -14)
#
# if(length(relay_prelims_3) == 0){
#   relay_prelims_3 <- rep(NA, length(rows))
# }
#
...
#
#
# i <- cut(relay_finals$Row_Numb, t(relay[,5:6]))
# levels(i)[c(FALSE,TRUE)] <- NA
# relay$Finals_Time[i] <- relay_finals$Finals_Time
#
# i <- cut(relay_prelims$Row_Numb, t(relay[,5:6]))
# levels(i)[c(FALSE,TRUE)] <- NA
# relay$Prelims_Time[i] <- relay_prelims$Prelims_Time
#
